...
export async function watchMachines(options: {
  ...
  const regenerate = (event: string, filePath: string) => {
    ...
    debounceTimer = setTimeout(async () => {
      console.log(`[app] ${event}: ${filePath} — regenerating…`);
      try {
        await generateAppGen({ ...options, cwd });
        console.log("[app] app.gen.ts updated.");
      } catch (err) {
  ...
  console.log("[app] Watching for machine file changes…");
...
export async function generateAppGen(options: {
  ...
  for (const f of files.sort()) {
    ...
    if (!info) {
      console.warn(
        `[app] Skipping (not statically evaluable): ${relative(cwd, f)}`,
      );
      continue;
    }
